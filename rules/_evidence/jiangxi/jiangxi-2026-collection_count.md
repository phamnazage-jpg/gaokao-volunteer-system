# jiangxi-2026-collection_count

> 对应规则: `JIANGXI.collection_count`
> 所属: 省级
> 规则版本: `2026.1`
> 摘录时间: 2026-06-18
> 摘录人: Hermes Agent (主代理)
> 证据状态: complete

## 1. 官方原文摘录

> "（2）本科批次。设特殊选拔单志愿和平行志愿2个志愿栏，特殊选拔单志愿栏设1个“院校专业组”，平行志愿栏设45个“院校专业组”。"
>
> "特殊选拔单志愿不安排征集志愿。平行志愿安排2次征集志愿，每次设20个“院校专业组”。"
>
> "（4）高职（专科）批次。设平行志愿，45个“院校专业组”。安排2次征集志愿，每次设20个“院校专业组”。"
> —— 出处: 《十七、普通类（历史、物理科目组）志愿如何设置？》
> URL: http://www.jxeea.cn/jxsjyksy/cjwd22/content/content_1856071499628613632.html
> 发布日期: 2024-06-21

## 2. 转写为机读规则

```yaml
JIANGXI.collection_count:
  severity: warning
  value:
    collection_count: 2
  effective_date: '2026-01-01'
  source_evidence_id: jiangxi-2026-collection_count
  status: active
```

## 3. 关键边界与例外

- 例 1：江西普通类本科批平行志愿已明确安排 `2次征集志愿`，因此 `collection_count` 不能再写成 `1`
- 例 2：提前本科批内部不同志愿栏的征集次数不完全一致，但项目当前机读值聚焦主批次，因此保留 `2` 作为普通类本科批口径

## 4. 后续维护

- 下次复核时间: 2026-07-20
- 复核来源: 江西
- 复核负责人: 待指派
